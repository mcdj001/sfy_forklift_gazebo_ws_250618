 ...
 * @Date: 2024-06-13 13:53:23
 * @LastEditors: Fengyuan Shen
 * @LastEditTime: 2024-06-26 18:29:19
 */
...
MPCNode::MPCNode()
{
    ...
    //my
    pn.param<bool>("steer_vel_delay_pid_model", steer_vel_delay_pid_model , true);
    pn.param<double>("steer_vel_delay_pid_kp",steer_vel_delay_pid_kp, 6);
    pn.param<double>("steer_vel_delay_pid_ki",steer_vel_delay_pid_ki, 0.01);
    pn.param<double>("steer_vel_delay_pid_kd",steer_vel_delay_pid_kd, 3);

    //Display the parameters
    cout << "\n===== MPC参数说明Parameters =====" << endl;
    ...
    // _sub_gen_path   = _nh.subscribe( "path", 1, &MPCNode::desiredPathCB, this);
    _sub_goal   = _nh.subscribe( _goal_topic, 1, &MPCNode::goalCB, this);
    _sub_amcl   = _nh.subscribe("/amcl_pose", 5, &MPCNode::amclCB, this);
    
    _pub_odompath  = _nh.advertise<nav_msgs::Path>("/mpc_reference", 1); // reference path for MPC  MPC预测的路径
    _pub_mpctraj   = _nh.advertise<nav_msgs::Path>("/mpc_trajectory", 1);// MPC trajectory output    MPC预测接下来mpc_steps个位姿
...
float MPCNode::z_vel_pid_controller(float pv,  float sp)
{
...
{          
    ...
    {    
        ...
        cout <<  "x_veh的size" << x_veh.size() << endl;
        // Fit waypoints
        ...
        if(steer_vel_delay_pid_model){
            const double expect_steer_vel =  mpc_results[0];
            _w = z_vel_pid_controller(odom.twist.twist.angular.z , expect_steer_vel);
        }else{
            _w = mpc_results[0];
        }
