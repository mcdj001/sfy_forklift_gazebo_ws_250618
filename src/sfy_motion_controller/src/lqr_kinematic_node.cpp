 ...
 * @Date: 2024-06-06 15:01:01
 * @LastEditors: Fengyuan Shen
 * @LastEditTime: 2024-06-26 19:08:29
 */

#include "lqr_kinematic_node.h"

vehicleState LQR_Kinematic_Node::update_state(U control, vehicleState forklift) {
	...
	forklift.yaw += forklift.v / L * tan(forklift.kesi) * T;
	forklift.yaw = mytools->normalizeAngle(forklift.yaw);
	return forklift;
}

vehicleState LQR_Kinematic_Node::delay_odom_postion(){
...
U LQR_Kinematic_Node::v_and_kesi_limit(U control_value){
...
void LQR_Kinematic_Node::Marker_set(){
    //设置消息类型参数
...
void LQR_Kinematic_Node::shutdown_controller(){
    if(action == "the car has reached the goal!"){
        temp+=1;
        if(temp ==50){
            ROS_WARN("shutdown the LQR Kinematic controller!");
            temp = 0;
            ros::shutdown();
        }
    }
...
float LQR_Kinematic_Node::z_vel_pid_controller(float pv,  float sp)
{
...
void LQR_Kinematic_Node::node_control() {
...
void LQR_Kinematic_Node::LQR_track() {
    U U_r;
    waypoint Point;
    ...
    if(path->Size()!=0){
        ...
        //使用LQR控制器,
        controller->initial(vehicle_dynamic_param.L, T, forklift_const, Point, U_r, Q, R);//初始化控制器
...
double LQR_Kinematic_Node::slow_judge(double distance , int cur_index , int all_index) {
...
void LQR_Kinematic_Node::addpointcallback(const nav_msgs::Path::ConstPtr& msg){
...
void LQR_Kinematic_Node::odomCallback(const nav_msgs::Odometry::ConstPtr& odom){
...
LQR_Kinematic_Node::~LQR_Kinematic_Node() {
    delete(controller);
    delete(path);
}


LQR_Kinematic_Node::LQR_Kinematic_Node(ros::NodeHandle& nh)//初始化中添加轨迹、小车初始位姿
{
    controller = new LQR_Kinematic_Control();
    path = new MyPath();
    
    //ROS:
    path_sub = nh.subscribe("desired_path", 10, &LQR_Kinematic_Node::addpointcallback, this);
    ...
    odomSub = nh.subscribe<nav_msgs::Odometry>("/odom", 10, &LQR_Kinematic_Node::odomCallback, this);
...
{
    setlocale(LC_ALL,"");
	ros::init(argc, argv, "LQR_Kinematic_Node");
	ros::NodeHandle n;
	...
	LQR_Kinematic_Node* node = new LQR_Kinematic_Node(n);
	node->node_control();
	return (0);
