 ...
 * @LastEditTime: 2024-10-17 16:40:00
 * @FilePath: /sfy_forklift_gazebo_ws/src/sfy_ros/sfy_planner/sfy_forklift_mission_planner/src/trajectory.cpp
...
void trajectory::generateLinePath(double x1, double y1, double x2, double y2) {
	genera_line_x_.clear();
    genera_line_y_.clear();
    // 计算两个点之间的距离
    double distance = sqrt(pow(x2 - x1, 2) + pow(y2 - y1, 2));

    // 计算插值点的数量
    int num_points = static_cast<int>(distance / line_distance_);
    
    // 计算步长
    double step_x = (x2 - x1) / distance * line_distance_;
    double step_y = (y2 - y1) / distance * line_distance_;

    // 生成路径
    for (int i = 0; i <= num_points; ++i) {
        genera_line_x_.push_back(x1 + i * step_x);
        genera_line_y_.push_back(y1 + i * step_y);
    }
}
...
void trajectory::mypath_bezier_plan_b(){
	...
	for(int i = 0 ; i < new_x.size() - 1 ; i++){
		if(new_turn[i] == 0 && new_turn[i+2] == 1){  // 贝塞尔曲线优化
			...
			// 贝塞尔曲线生成
			for(int t = 0; t < curve_num_; t++){
        		Vector2d pos = mytools->bezierCommon(Ps,(double)t/curve_num_);
				PP.ID = k++;
				PP.x = pos[0];
				PP.y = pos[1];
				waypoints.push_back(PP);
			}
			vector<Vector2d> bezier_waypoints;
			// mytools->generateBezierPath(Ps, line_distance_, bezier_waypoints);
			// for (int t = 0; t < waypoints.size(); ++t) {
			// 	PP.ID = k++;
			// 	PP.x = bezier_waypoints[t][0];
			// 	PP.y = bezier_waypoints[t][1];
			// 	waypoints.push_back(PP);
     		//    	cout << "Point " << t << ": (" <<  bezier_waypoints[t][0] << ", " << bezier_waypoints[t][1] << ")\n";
    		// }
		...
		if(new_turn[i] == 0 && new_turn[i+1] == 0){   // 生成直线
			...
			// 	waypoints.push_back(PP);
			// }
			generateLinePath(new_x[i], new_y[i], new_x[i+1], new_y[i+1]);
			for(int j = 0; j < genera_line_x_.size(); ++j) {
				PP.ID = k++;
				PP.x = genera_line_x_[j];
				PP.y = genera_line_y_[j];
				waypoints.push_back(PP);
			}
    ...
    // }
	// cout <<    "======================="<<endl;
	double last_k = 0 , sum_k = 0 , count_k_use = 0;
	...
	double p_max =   tan(60.f / 180.f * M_PI) / 1.48;

	// cout << endl<<  "平均曲率为：" << sum_k / count_k_use << "  p_max:  " << p_max<< endl;	
}
